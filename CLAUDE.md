...
- Theme is system-aware with an explicit picker (Settings → Theme) + a quick light↔dark topbar toggle (hidden while a committed theme like Midnight Neon is active); **dark is the default** for an unset preference — resolving to **Midnight Neon** (`preferences.theme` default, the `SYSTEM_DARK` id in `theme.js`, and the boot script in `index.html` must agree).
